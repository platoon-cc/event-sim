...
func New(key string) (*Processor, error) {
	...
	p.insert_stmt, err = p.db.Prepare(`INSERT INTO events (id,event,user_id,timestamp,params) VALUES (?,?,?,?,?)`)
	if err != nil {
		return nil, err
	}
...
func (p *Processor) StoreEvents(events []model.Event, idOffset int64) error {
	for _, e := range events {
		...
		fmt.Printf("%d %s %s \t%s \t%v\n", eventId, t, e.UserId, e.Event, e.Params)
		if _, err := p.insert_stmt.Exec(eventId, e.Event, e.UserId, e.Timestamp, e.Params); err != nil {
			return err
		}
	}
	return nil
...
func (p *Processor) Query() error {
	rows, err := p.db.Query(`select params ->> 'name' as key, avg(params ->> 'score') from events where event='challengeEnd' group by key;`)
	if err != nil {
		return err
	}
